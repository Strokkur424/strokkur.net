...
## Paper-specific argument annotations
The following annotations are available specifically for Paper plugins:

### @AngleArg
Declares that a `float` parameter should be interpreted as an angle argument:
```java
@Executes
void rotate(CommandSender sender, @AngleArg float angle) {
  // angle is a rotation value in degrees
}
```

### @TimeArg
Declares that an `int` parameter should be interpreted as a time argument (duration in ticks):
```java
@Executes
void wait(CommandSender sender, @TimeArg int ticks) {
  // ticks represents a duration, like in /time set
}

// You can also specify a minimum time value:
@Executes
void waitAtLeast(CommandSender sender, @TimeArg(20) int ticks) {
  // ticks must be at least 20 (1 second)
}
```

### @FinePosArg
Configures a `FinePosition` argument. The value specifies whether integer input gets centered to `.5`:
```java
@Executes
void teleport(CommandSender sender, @FinePosArg(true) FinePosition pos) {
  // If true, integer x/z values are centered to .5
  // If false (default), they're left as .0 (block corner)
}
```

### @CustomArg
Allows you to use Paper's `CustomArgumentType` for custom argument types:
```java
@Executes
void execute(CommandSender sender, @CustomArg(MyCustomArgument.class) CustomType value) {
  // value is parsed by MyCustomArgument which implements CustomArgumentType<CustomType, ?>
}
```
