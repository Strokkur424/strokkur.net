...
## The `@DefaultExecutes` annotation
This annotation has the exact same syntax as `@Executes`. It behaves the same way (in fact, you can annotate
the same method both `@Executes` *and* `@DefaultExecutes`) and takes in the same parameter input for a preceding
literal path.
