...
Permissions are a very significant part of making sure that a command is not executed by people
...
The permission annotations may also be referred to as "requirement annotations", since they add a `.requires` clause
to the command tree.
