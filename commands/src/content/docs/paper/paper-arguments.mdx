...
Certain argument types *usually* have to be resolved by the developer. A very well-known example of that is the
...
### Signed key argument type
If you want to reference an [`ArgumentTypes.signedMessage()`](https://jd.papermc.io/paper/io/papermc/paper/command/brigadier/argument/ArgumentTypes.html#signedMessage()),
you have to add a `CompletableFuture<SignedMessage>`. This follows the exact same logic as the other resolver methods,
but as it might come as a surprise, hence the explicit addition in the documentation.
...
the [`RegistryKey<T>`](https://jd.papermc.io/paper/io/papermc/paper/registry/RegistryKey.html) JavaDocs.
...
in a [`TypedKey<T>`](https://jd.papermc.io/paper/io/papermc/paper/registry/TypedKey.html).
