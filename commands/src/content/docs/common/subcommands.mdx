...
which allow you to nest classes, annotated with the `@Subcommand` annotation to add preceding literals to
any executors (or further nested classes!) inside the nested class.

## Syntax
The syntax is basic. An example nested command might look like this:
...
There are many ways to write a command. Nested/inner classes (as they are called officially) are just another
...
## Inner records
The more interesting use case of subcommands like this is inner records. With records, you can also define arguments.
